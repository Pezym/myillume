...
const Shop = () => {
  ...
  const handleAddBundle = async (bundle: typeof bundlePricing['3-in-1-oral-kit'][0]) => {
    const { product, variantId } = buildBundleCartProduct(bundle.qty, bundle.label, bundle.price, toothbrushImg);
    await addItem({
      product,
      variantId,
      variantTitle: bundle.label,
      price: { amount: bundle.price.toFixed(2), currencyCode: 'USD' },
      quantity: 1,
      selectedOptions: [],
    });
  };
        ...
        <section className="max-w-7xl mx-auto px-6 pb-10">
          <div className="group relative grid grid-cols-1 md:grid-cols-2 bg-sand-light/30 rounded-3xl overflow-hidden border border-border hover:shadow-xl transition-shadow duration-300">
            <Link to={`/product/${featured.id}`} className="aspect-square md:aspect-auto overflow-hidden bg-white">
              <img src={featured.image} alt={featured.name} className="w-full h-full object-contain p-8 group-hover:scale-105 transition-transform duration-700" />
            </Link>
            <div className="flex flex-col justify-center p-8 md:p-12">
              ...
              <button
                ...
                disabled={isLoading}
                className="self-start flex items-center gap-2 bg-primary text-primary-foreground font-body text-xs tracking-widest uppercase px-8 py-3.5 rounded-full hover:bg-sand hover:text-primary transition-colors disabled:opacity-50"
              >
                ...
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-white mb-4">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                    <button
                      ...
                      disabled={isLoading}
                      className="w-full flex items-center justify-center gap-2 bg-primary/95 backdrop-blur text-primary-foreground font-body text-[10px] tracking-widest uppercase py-3 rounded-full hover:bg-sand hover:text-primary transition-colors disabled:opacity-50"
                    >
            ...
            {bundlePricing['3-in-1-oral-kit'].map(bundle => (
              ...
              >
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full bg-sand text-primary font-body text-xs tracking-widest uppercase py-3 rounded-full hover:bg-gold transition-colors disabled:opacity-50"
                >
                  Add to Cart
                </button>
              </div>
